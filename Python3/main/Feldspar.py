...
#Normalizing the wt% and then calculate the anion ratio on the basis of 6O
    ...
    data_cali = {
                ...
                'CaO': float("%.3f" % CaO_wt),
                'K2O': float("%.3f" % K2O_wt),
                'Na2O': float("%.3f" % Na2O_wt),
                'Total': float("%.3f" % sum_wt),
                'Comment': data['Comment'],
                'Si': float("%.4f" % SiO2_co2), 
                'Ti': float("%.4f" % TiO2_co2),
                'Al': float("%.4f" % Al2O3_co2),
                'Al(4)': float("%.4f" % Al4),
                'Al(6)': float("%.4f" % Al6),
                'Fe3+': float("%.4f" % Fe2O3_co2),
                'Fe2+': float("%.4f" % FeO_co2),
                'Mn': float("%.4f" % MnO_co2),
                'Mg': float("%.4f" % MgO_co2),
                'Ca': float("%.4f" % CaO_co2),
                'K': float("%.4f" % K2O_co2),
                'Na': float("%.4f" % Na2O_co2),
                'Or': float(K2O_co2 * 100 / (K2O_co2 + Na2O_co2 + CaO_co2)),
                'Ab': float(Na2O_co2 * 100 / (K2O_co2 + Na2O_co2 + CaO_co2)),
                'An': float(CaO_co2 * 100 / (K2O_co2 + Na2O_co2 + CaO_co2)),
                }
      
    return pd.DataFrame(data_cali, index = [data_index], columns = ['SiO2', 'TiO2', 'Al2O3', 'Fe2O3', 'FeO', 'MnO', 'MgO', 'CaO', 'K2O', 'Na2O', 'Total', 'Comment', 'Si', 'Ti', 'Al', 'Al(4)', 'Al(6)', 'Fe3+', 'Fe2+', 'Mn', 'Mg', 'Ca', 'K', 'Na', 'Or', 'Ab', 'An'])
